...
}, messageCacheMaxSize: 0, disableMentions: 'everyone'});
	...
	try {
		...
		if(messages.size !== 0) {
			...
			if(channelConfig.ttl < /*6 days*/518400000) {
				...
				if(messages.size !== 0)
					interval = Math.max(messages[0].createdTimestamp + channelConfig.ttl - now, DayMs);
				else
...
function AddChannel(matches, channel, announce) {
	...
	if(announce) {
		DeleteOldAnnounce(channel).then(() =>
			channel.send("The messages will be deleted after " + ((param === 1) ? "one day" : param + " days")).then(message => message.pin().catch()).catch()
		);
	}
}
...
const DeletebotRegex = /DeleteBot\s*(\d{1,3})?/ig;
