...
async function Wipe(channelConfig, reWipe) {
  ...
  channelConfig.t = setTimeout(Wipe, interval + crypto.randomInt(2) * RecurringSpread, channelConfig);
}
...
function AddChannel(matches, channel, announce) {
  ...
  channelConfig.t = setTimeout(Wipe, crypto.randomInt(2) * InitialSpread, channelConfig, true);
