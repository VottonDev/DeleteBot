...
module.exports = {
  data : new SlashCommandBuilder()
           .setName('add')
           .setDescription(
               'Add a channel that you want to automatically delete messages on')
           .addChannelOption(
               (option) =>
           ...
           .addIntegerOption(
               (option) =>
                   option.setName('days')
                       .setDescription(
                           'The number of days before messages are deleted')
                       .setRequired(false)),
       async execute(interaction) {
         ...
         if (!interaction.member.permissions.has('MANAGE_CHANNELS')) {
           ...
           await interaction.reply(
               'Successfully added channel to list of channels that automatically delete messages.');
           // Add channel topic
           ...
           if (interaction.options.getInteger('days') > 12) {
             ...
             channel.setTopic(
                 `DeleteBot ${interaction.options.getInteger('days') || 7}`);
           }
         }
       },
};
